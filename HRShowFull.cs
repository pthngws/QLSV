...
namespace QLSV
{
    public partial class HRShowFull : Form
    {
        ...
        private void listBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            SqlCommand command = new SqlCommand("SELECT contact.id, fname, lname, name, phone, email, address, picture, contact.userid FROM contact,[group] WHERE [group].id = contact.groupid and [contact].userid = @id and groupid = @gid", mydb.getConnection);
        ...
        {
            SqlCommand command = new SqlCommand("SELECT contact.id, fname, lname, name, phone, email, address, picture, contact.userid FROM contact,[group] WHERE [group].id = contact.groupid and [contact].userid = @id", mydb.getConnection);
            command.Parameters.Add("@id", SqlDbType.VarChar).Value = Global.GlobalUserID;
            dataGridView1.DataSource = contac.GetContact(command);
